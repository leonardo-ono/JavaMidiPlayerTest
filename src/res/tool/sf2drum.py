    ...
    for index, bag in enumerate(instrument.bags):
        ...
        for bag_id, bag_gen in bag.gens.items():
            if bag_id == 53: # sample
                ...
                print("drum_note=", bag.key_range[0])
                ...
                export_sample_to_wav(instrument_sample, "samples/drums/" + instrument_sample.name + ".wav")
...
def main():
    ...
    for index, preset in enumerate(sf2.presets):
        ...
        if hasattr(preset, 'bank') and hasattr(preset, 'preset'):
            # drum kit
            if preset.bank == 128 and preset.preset == 0:
                print("# preset=", preset.bank, ":", preset.preset, "-", preset.name, "-")
                for bag in preset.bags:
                    for value, gen in bag.gens.items():
                        if value == 41:
                            instrument_id = gen.amount # preset.bags[3].gens.get(41).amount
